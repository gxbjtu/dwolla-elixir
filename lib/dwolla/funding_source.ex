defmodule Dwolla.FundingSource do
  ...
  """
  @spec initiate_micro_deposit(token, id) :: {:ok, any()} | {:error, error}
  def initiate_micro_deposit(token, id) do
    endpoint = @endpoint <> "/#{id}/micro-deposits"

    Dwolla.make_request_with_token(:post, endpoint, token, %{}, Map.new())
    |> Utils.handle_resp(:empty)
  end

  @doc """
  Verify micro-deposit.
  """
  @spec verify_micro_deposit(token, id, number(), number()) ::
          {:ok, any()} | {:error, error}
  def verify_micro_deposit(token, id, amount1, amount2) do
    endpoint = @endpoint <> "/#{id}/micro-deposits"
